...
function ImageSlideshow({ images, title }: { images: ResponsiveImageData[]; title: string }) {
  ...
  const keepThumbnailVisible = useCallback(
    (nextIndex: number) => {
      const total = images.length * THUMB_STEP - THUMB_GAP;
      const max = Math.max(0, total - ribbonViewportWidth);
      const thumbnailStart = nextIndex * THUMB_STEP;
    ...
    [images.length, ribbonViewportWidth],
  );
  ...
  const showImage = useCallback(
    ...
    [current, fading, keepThumbnailVisible],
  );
  ...
  const go = useCallback(
    ...
    [current, images.length, showImage],
  );
  ...
  const totalRibbonWidth = images.length * THUMB_STEP - THUMB_GAP;
  const maxOffset = Math.max(0, totalRibbonWidth - ribbonViewportWidth);
  const scrollRibbon = (dir: 1 | -1) => {
    setRibbonOffset((o) => Math.max(0, Math.min(maxOffset, o + dir * THUMB_STEP * 3)));
  };

  const renderThumbnailRibbon = () => (
    <div
      style={{
        display: "flex",
        alignItems: "center",
        ...
        >
          {images.map((src, i) => (
            <button
              ...
              onMouseEnter={(e) => {
                if (i !== current) e.currentTarget.style.opacity = "0.8";
              }}
              onMouseLeave={(e) => {
                if (i !== current) e.currentTarget.style.opacity = "0.45";
              }}
            >
              <ResponsiveImage
                src={src}
  ...
  return (
    <>
      <div style={{ maxWidth: "1280px", margin: "0 auto", userSelect: "none" }}>
        ...
        {images.length > 1 && renderThumbnailRibbon()}
      </div>
...
export default function ProjectDetail({ slug, navigate }: Props) {
  ...
  const slideshowImages = images.length > 1 ? [images[1], images[0], ...images.slice(2)] : images;
  ...
  const meta = [
    ...
    ...(project.gfa
      ? [
          {
            label: zh ? "总建筑面积" : "gross floor area (gfa)",
            value: project.gfa,
          },
        ]
      : []),
