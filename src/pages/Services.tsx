...
export default function Services({ navigate }: Props) {
  ...
  useEffect(() => {
    ...
    const scrollToSlide = (index: number) => {
      ...
      transitionTimer = window.setTimeout(() => { transitionActive = false }, 650)
      window.scrollTo({
    ...
    const holdWheelGesture = () => {
      ...
      wheelGestureTimer = window.setTimeout(() => { wheelGestureActive = false }, 180)
    }
    ...
    const handleWheel = (event: WheelEvent) => {
      ...
      if (!isPinned(pinStart, pinEnd)) {
        const crossesStart = deltaY > 0 && window.scrollY < pinStart && window.scrollY + deltaY >= pinStart
        const crossesEnd = deltaY < 0 && window.scrollY > pinEnd && window.scrollY + deltaY <= pinEnd
    ...
    const handleTouchMove = (event: TouchEvent) => {
      ...
      if (!isPinned(pinStart, pinEnd)) {
        const enteringFromAbove = direction > 0 && window.scrollY < pinStart && rect.top <= window.innerHeight
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const movesDown = event.key === 'ArrowDown' || event.key === 'PageDown' || (event.key === ' ' && !event.shiftKey)
      const movesUp = event.key === 'ArrowUp' || event.key === 'PageUp' || (event.key === ' ' && event.shiftKey)
