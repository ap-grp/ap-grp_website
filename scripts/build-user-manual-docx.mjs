...
if (!fs.existsSync(logoSource)) {
  await sharp(path.join(root, 'src', 'assets', 'apgrp_logo.svg')).resize({ width: 700 }).png().toFile(logoSource)
}
...
const orderedNumIds = [...new Set(blocks.filter((block) => block.numId).map((block) => block.numId))]
const hyperlinks = []
...
for (const block of blocks) {
  ...
  if (block.type === 'heading') {
    ...
    const style = coverMode
      ? (block.level === 1 ? 'Title' : 'Subtitle')
      : `Heading${block.level}`
    bodyXml.push(paragraph(block.text, { style, align: coverMode ? 'center' : undefined, keepNext: true }))
    continue
  }
  ...
  if (block.type === 'listItem') {
    ...
    const displayText = isChecklist
      ...
      : block.text
    bodyXml.push(paragraph(displayText, {
...
function inlineRuns(text) {
  ...
  for (const match of text.matchAll(tokenPattern)) {
    ...
    if (token.startsWith('**')) {
      ...
      parts.push(`<w:hyperlink r:id="${relationshipId}" w:history="1">${run(label, { hyperlink: true })}</w:hyperlink>`)
    }
...
function run(text, options = {}) {
  ...
  if (options.code) properties.push('<w:rFonts w:ascii="Consolas" w:hAnsi="Consolas"/><w:color w:val="8A684A"/><w:shd w:val="clear" w:color="auto" w:fill="F5F0EB"/><w:sz w:val="18"/><w:szCs w:val="18"/>')
...
function codeParagraph(text) {
  ...
  const content = lines.map((line, index) => `${run(line || ' ', { code: true })}${index < lines.length - 1 ? '<w:r><w:br/></w:r>' : ''}`).join('')
...
function tableXml(rows) {
  ...
  const rowXml = rows.map((row, rowIndex) => {
    const cells = Array.from({ length: columnCount }, (_, columnIndex) => row[columnIndex] || '')
      .map((cell) => `<w:tc><w:tcPr><w:tcW w:w="${cellWidth}" w:type="dxa"/>${rowIndex === 0 ? '<w:shd w:val="clear" w:color="auto" w:fill="212529"/>' : ''}<w:tcMar><w:top w:w="90" w:type="dxa"/><w:left w:w="110" w:type="dxa"/><w:bottom w:w="90" w:type="dxa"/><w:right w:w="110" w:type="dxa"/></w:tcMar></w:tcPr><w:p><w:pPr><w:pStyle w:val="TableText"/></w:pPr>${rowIndex === 0 ? run(cell, { bold: true, color: 'FFFFFF' }) : inlineRuns(cell)}</w:p></w:tc>`)
      .join('')
    return `<w:tr><w:trPr><w:cantSplit/>${rowIndex === 0 ? '<w:tblHeader/>' : ''}</w:trPr>${cells}</w:tr>`
  }).join('')
...
function documentRelationships() {
  const fixed = [
    ...
    ['rId5', 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/header', 'header1.xml'],
    ['rId6', 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/footer', 'footer1.xml'],
    ['rId7', 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/image', 'media/logo.png'],
  ]
  const relationships = fixed.map(([id, type, target]) => `<Relationship Id="${id}" Type="${type}" Target="${target}"/>`)
  relationships.push(...hyperlinks.map(({ id, url }) => `<Relationship Id="${id}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink" Target="${xmlAttribute(url)}" TargetMode="External"/>`))
...
function numberingXml(orderedNumIds) {
  const orderedInstances = orderedNumIds
    .map((numId) => `<w:num w:numId="${numId}"><w:abstractNumId w:val="1"/><w:lvlOverride w:ilvl="0"><w:startOverride w:val="1"/></w:lvlOverride></w:num>`)
    .join('')
